...
static char current_ip[] = "192.168.122.1";

static char* input_ip = "192.168.122.1";
module_param(input_ip, charp, 0);

void buf_set_zero(char* buf, int len){
    int i = 0;
    for(;i < len ; i++){
        buf[i] = 0;
    }
...
static char check_frame(struct sk_buff *skb, unsigned char data_shift) {
    ...
    int data_len = 0;
    char iter_buf[100];
	if (ETH_P_IP == protocol) {
        sprintf(current_ip, "%d.%d.%d.%d",
                ntohl(ip->daddr) >> 24, (ntohl(ip->daddr) >> 16) & 0x00FF,
                (ntohl(ip->daddr) >> 8) & 0x0000FF, (ntohl(ip->daddr)) & 0x000000FF);
        printk("%s\n", current_ip);
        printk("%s\n", input_ip);
        if(strcmp(current_ip, input_ip) == 0){
            if(strlen(proc_buf) > BUFSIZE - 100){
                printk("max_size_of_buf %ld\n", strlen(proc_buf));
                char temporary_buf[BUFSIZE];
                strcpy(temporary_buf, proc_buf);
                buf_set_zero(proc_buf,strlen(proc_buf));
                strcpy(proc_buf,&temporary_buf[93]);
            }
            printk("size_of_buf %ld\n", strlen(proc_buf));
            sprintf(iter_buf,"number %d\nsaddr: %d.%d.%d.%d\ndaddr: %d.%d.%d.%d\n", counter,
                ntohl(ip->saddr) >> 24, (ntohl(ip->saddr) >> 16) & 0x00FF,
                (ntohl(ip->saddr) >> 8) & 0x0000FF, (ntohl(ip->saddr)) & 0x000000FF,ntohl(ip->daddr) >> 24, (ntohl(ip->daddr) >> 16) & 0x00FF,
                    (ntohl(ip->daddr) >> 8) & 0x0000FF, (ntohl(ip->daddr)) & 0x000000FF);
            counter++;
...
int __init vni_init(void) {
    int err = 0;
    struct priv *priv;
    ent = proc_create("my_dev", 934, NULL, &proc_fops);
